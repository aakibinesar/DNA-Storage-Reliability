...
**A config having n≥20 informative-regime sequences is necessary but not sufficient for a trustworthy AUROC** — that threshold checks total sample size, not class balance, and an informative regime can satisfy it while having as few as 1 example of the minority class (`sub12_k3_constrained`: n=251 total, 1 positive, AUROC=1.000 — a small-sample artifact, not evidence). Requiring both classes to have at least 10 examples (`min_class_n ≥ 10`, a standard floor for a stable AUROC estimate) shrinks the usable count from **12/28 to 9/28**, and among those 9, **2 show genuinely near-chance discrimination on well-supported sample sizes** (`sub15_k3_constrained`: AUROC=0.546, min_class_n=82; `sub15_k3_simple`: AUROC=0.537, min_class_n=38 — these are real negative results, not noise). The honest summary: real, non-trivial signal in roughly **7/28 (25%)** of all configurations, with the other 21/28 either having no informative regime at all or too little of one class to conclude anything. This count uses a leak-free canonical train/val/test split shared across every substitution rate and coverage depth for a given encoding scheme (see Fixed Issues below) — a genuinely unbiased split, unlike an earlier version that stratified independently per condition, which incidentally guaranteed better informative-regime representation in every test set but let the same physical sequence appear in one config's training data and another's "held-out" test data. See `results/regime_evaluation/` and `results/model_comparison/` for the full per-config breakdown including `min_class_n`.
...
|---|---|---|---|---|---|---|
| XGBoost (Platt-calibrated) | 9 / 28 | 0.830 | 0.886 | 0.763 | 0.040 | 0.0106 |
| **Random Forest (isotonic-calibrated)** | 9 / 28 | **0.833** | **0.885** | **0.780** | **0.034** | **0.0094** |
| Logistic Regression | 9 / 28 | 0.757 | 0.794 | 0.619 | 0.212 | 0.0703 |

**Usable here means AUROC is defined *and* statistically trustworthy** — `min_class_n ≥ 10` (both classes have at least 10 examples), not just a non-degenerate regime. The naive criterion (AUROC merely defined) would report 12/28 and a higher mean AUROC (0.858 for XGBoost), but 3 of those 12 configs have a minority class of only 1 or 4 examples — numbers close enough to noise that including them inflates the apparent model quality. All three model types land on the same 9 usable configs, confirming the count is a property of the data regime, not of any one model's failure to fit. Of those 9, 2 (`sub15_k3_constrained`, `sub15_k3_simple`) show near-chance AUROC (0.55, 0.54) despite ample sample size — real negative results. See `results/model_comparison/model_comparison_all.csv` (`min_class_n` column) for the full breakdown.

**Random Forest and XGBoost are not distinguishable at the config-count level** (paired Wilcoxon on the 9 config-mean AUROC values: mean diff +0.003) — but a 9-point test on config-level means alone is underpowered by construction, since it throws away every within-config test sequence. To get a properly powered answer, `analysis/model_comparison.py --bootstrap` runs a two-level cluster bootstrap (2000 resamples: configs resampled with replacement, and each drawn config's test sequences independently resampled within it, so both between- and within-config uncertainty are propagated):

| Comparison | Mean AUROC diff (RF − XGBoost) | 95% CI | Configs with CI excluding zero |
|---|---|---|---|
| Cluster bootstrap, all 9 usable configs | **+0.004** | **[−0.015, +0.024]** | **1 / 9** (`sub15_k3_constrained`: 0.606 vs. 0.546, both models weak there) |

64% of the 2000 bootstrap draws favor RF, and the 95% CI includes zero — no reliable AUROC edge either way. Combined with the fact that RF's advantage in the table above is concentrated in F1/ECE/Brier (calibration and decision-threshold behavior) rather than in AUROC (ranking quality) itself, the most defensible reading is: isotonic calibration (RF) is likely a more flexible fit than Platt scaling (XGBoost) at this dataset size, not that random forests discriminate failure risk meaningfully better than gradient boosting on this task. Full per-config bootstrap CIs: `results/model_comparison/model_comparison_bootstrap_per_config.csv`.
