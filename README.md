...
The channel model (`src/channel_model.py`) is explicitly built on **DeSP** (Yuan, Xie, Wang & Wang, "DeSP: a systematic DNA storage error simulation pipeline," *BMC Bioinformatics*, 2022) — same Synthesis → Decay → PCR → Sampling → Sequencing stage structure, and the fixed (non-swept) per-stage parameters are set at DeSP's own reported values or order of magnitude: `synthesis_sub_rate=0.001`, `indel_rate=0.001` (cited to Seo et al. Fig S8 in `configs/experiment_config.yaml`), `sequencing_sub_rate=0.002` ("Illumina default"), `pcr_fidelity=0.9999`. These line up with DeSP's own per-stage rates (synthesis ~10⁻⁶–10⁻⁵, sequencing substitution noise an order or two higher).

**The swept variable — substitution rate from 1% to 20% — is a much larger range than any single per-base error rate above, and is the one part of the setup that most needs an external check.** DeSP's own Figure 2 reports *final* (post-pipeline, pre-decode) error rates at reduced sequencing depth/sampling ratio reaching into the same double-digit range this project tests (their reported figures include values around 16–20% at low sampling ratios, alongside single-digit values at higher ones). That doesn't validate every point on this project's sweep individually, but it does mean **the upper end of the tested range (sub18–20%) corresponds to a realistic degraded/low-coverage regime reported in the same source paper this simulator is built on, not an arbitrary stress-test extreme invented for this project.** The "Over-failure" regime at that end of the sweep (Failure Regime Map above) should be read as "realistic under low-coverage/degraded conditions," and the informative "sweet spot" band in between as the more moderate, more commonly-expected operating range.

*(Caveat on this specific check: DeSP's Figure 2 was read via automated PDF text extraction, not manually verified table-cell-by-table-cell — the qualitative conclusion (final error rates DO reach the double digits in DeSP's own reported data at reduced sampling) is solid, but a manual read of the source figure is worth doing before citing exact percentages from it in the manuscript.)*
