...
| `train` | Train and calibrate all models across 16 configs |
| `gate_check` | Validate model quality before allocation experiments |
| `allocation` | Run adaptive vs. uniform vs. oracle allocation (48 experiments) |
| `ablation` | Feature group ablation across all 16 configs |
...
- **Single-class guard**: at low substitution rates and high coverage, virtually no sequences fail. A `DummyClassifier` is used in these cases rather than forcing a meaningless classifier
